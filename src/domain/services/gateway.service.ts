...
export class GatewayService {
  ...
  async processRequest(request: RequestEntity): Promise<any> {
    ...
    try {
      ...
      const requireAuthForPlaces = !authDisabled && (serviceRoute === 'places' || serviceRoute === 'routes' || serviceRoute === 'recommender' || serviceRoute === 'logbook' ||
        serviceRoute === 'favorites' || serviceRoute === 'visited'); 
  ...
  private getServiceUrl(serviceRoute: string): string {
    const serviceUrls = {
      ...
      recommender: process.env.RECOMMENDER_SERVICE_URL || 'http://localhost:8080',
      popularity: process.env.RECOMMENDER_SERVICE_URL || 'http://localhost:8080',
      logbook: process.env.LOGBOOK_SERVICE_URL || 'http://localhost:3003', 
    };
